...
from Week1.cipher import SimpleStreamCipher
from Week1.hacker import Hacker
from Week1.helpers import strxor

MSGS = [
    # 10 plaintext messages
]
...
def main():
    # Encoding MSGS
    # key = random(1024)
    # cipher = SimpleStreamCipher()
    # ciphertexts = [cipher.encrypt(key, bytes.fromhex(msg)) for msg in MSGS]

    # Try to hack cpyto-messages
    hacker = Hacker([bytes.fromhex(ctext) for ctext in ciphertexts], 1024)
    known_key_positions, guess_key = hacker.two_time_pad_attack()
    print('guess_key: {}'.format(guess_key))
    guess_output = strxor(bytes.fromhex(target_cipher), guess_key)
    guess_output = ''.join(chr(char) if index in known_key_positions else '*' for index, char in enumerate(guess_output))
    print('guess_output:    {}'.format(guess_output))

    # After this fill missed gaps in output message ('*') on appropriate symbols
    # and get real_key without missed gaps
    target_text = "The secret message is: When using a stream cipher, never use the key more than once"
    real_key = strxor(bytes(target_text, 'utf-8'), bytes.fromhex(target_cipher))
    output_c1 = strxor(bytes.fromhex(c1), real_key)
    print('output_c1: {}'.format(str(output_c1)))
